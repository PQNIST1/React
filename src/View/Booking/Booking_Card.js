...
export default function Card({idData, hotelData, role }) {
    const [status, setStatus] = useState(hotelData.status);
    const updateBookingStatus = async () => {
        setStatus(!hotelData.status);
        try {
            const response = await axios.put(
                'http://localhost:8000/user/booking',
                {
                    userId: idData, 
                    bookingId: hotelData.hotel._id, 
    ...
    const handDetail = () => {
        role === 'admin' ?window.location.href = `/booking`:window.location.href = `/booking/detail/${hotelData.hotel._id}`;
    };
    return (
        <>      
        <div className="hotel-card">
            <picture className='picture'>
                <img src={hotelData.hotel.image[0]}></img>
            </picture>
            <div className='card-content'>
                <h4>{hotelData.hotel.name}</h4>
            ...
            <button className='btn btn-primary' type='button' onClick={updateBookingStatus}>Xác nhận phòng</button>      
            ): (
                <button className='btn btn-primary' type='button'  onClick={handDetail}>Xem hoặc cập nhật chi tiết</button>
            )}
            </div>
        </div>
    </>
